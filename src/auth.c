...
const uint8_t BroadcastAddr[6] = {0xff, 0xff, 0xff,
                                  0xff, 0xff, 0xff};  // 广播MAC地址
const uint8_t MultcastAddr[6] = {0x01, 0x80, 0xc2,
                                 0x00, 0x00, 0x03};  // 多播MAC地址
char H3C_VERSION[16];
char H3C_KEY[64];
// const char H3C_VERSION[16]="CH\x11V7.10-0313"; // 华为客户端版本号
// const char H3C_KEY[64]    ="HuaWei3COM1X";  // H3C的固定密钥
// const char H3C_KEY[64]  ="Oly5D62FaE94W7";  //
// H3C的另一个固定密钥，网友取自MacOSX版本的iNode官方客户端
...
static void SendFirstResponseIdentity(pcap_t *adhandle, const uint8_t request[],
                                      const uint8_t ethhdr[],
                                      const uint8_t ip[4],
                                      const char username[]);
static void SendResponseMD5(pcap_t *adhandle, const uint8_t request[],
                            const uint8_t ethhdr[], const char username[],
                            const char passwd[]);
static void SendResponsePassword(pcap_t *handle, const uint8_t request[],
                                 const uint8_t ethhdr[], const char username[],
                                 const char passwd[]);
static void SendResponseAvailable(pcap_t *adhandle, const uint8_t request[],
                                  const uint8_t ethhdr[], const uint8_t ip[4],
                                  const char username[]);
...
// From fillmd5.c
extern void FillMD5Area(uint8_t digest[], uint8_t id, const char passwd[],
...
START_AUTHENTICATION: {
  ...
  // 分情况应答下一个包
  if ((EAP_Type)captured[22] ==
      IDENTITY) {  // 通常情况会收到包Request Identity，应回答Response Identity
  ...
  } else if ((EAP_Type)captured[22] ==
             AVAILABLE) {  // 遇到AVAILABLE包时需要特殊处理
    // 中南财经政法大学目前使用的格式：
  ...
  // 重设过滤器，只捕获华为802.1X认证设备发来的包（包括多播Request Identity /
  // Request AVAILABLE）
  sprintf(
      FilterStr,
      "(ether proto 0x888e) and (ether src host %02x:%02x:%02x:%02x:%02x:%02x)",
      captured[6], captured[7], captured[8], captured[9], captured[10],
      captured[11]);
  pcap_compile(adhandle, &fcode, FilterStr, 1, 0xff);
  pcap_setfilter(adhandle, &fcode);
      ...
      switch ((EAP_Type)captured[22]) {
        ...
        case ALLOCATED:  // newtype:7;EAP_REQUEST ALLOCATED PACKAGE;
          DPRINTF("[%d] Server: Request Allocated(Password)!\n",
                  (EAP_ID)captured[19]);
          SendResponsePassword(adhandle, captured, ethhdr, UserName,
      ...
      if (errtype == 0x09 && msgsize > 0) {  // 输出错误提示消息，gbk 编码转换
        ...
        if (mode == 1) {
          ...
          return 0;  // exit(-1);
                 ...
                 0x08)  // 可能网络无流量时朊务器结束此次802.1X认证会话
...
static void SendResponseAvailable(pcap_t *handle, const uint8_t request[],
                                  const uint8_t ethhdr[], const uint8_t ip[4],
                                  const char username[]) {
  int i;
  uint16_t eaplen;
...
void SendFirstResponseIdentity(pcap_t *adhandle, const uint8_t request[],
                               const uint8_t ethhdr[], const uint8_t ip[4],
                               const char username[]) {
  uint8_t response[128];
  size_t i;
...
static void SendResponsePassword(
    pcap_t *handle, const uint8_t request[], const uint8_t ethhdr[],
    const char username[],
    const char passwd[])  // SendResponsePassword(adhandle, captured, ethhdr,
                          // UserName, Password);
{
  uint16_t eaplen;
  size_t usernamelen;
  ...
  packetlen =
      24 + usernamelen +
      passwordlen;  // 14+4+22+usernamelen; // ethhdr+EAPOL+EAP+usernamelen
...
// （注：该函数也可反向用于解密）
static void XOR(uint8_t data[], unsigned dlen, const char key[],
                unsigned klen) {
  unsigned int i, j;
