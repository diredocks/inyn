...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3 || argc > 6 || getuid() != 0) {
    fprintf(
        stderr,
        "Usage: %s [USERNAME] [PASSWORD]... [DEVICE] [CLIENT-VERSION] [KEY]\n",
        argv[0]);
    fprintf(stderr, "\nRoot privilege is needed to capture network frames\n");
    fprintf(stderr, "If no device given, 'enp1s0' would be the default\n");
    fprintf(stderr,
            "When typing client veriosn with hex character, please do it with "
            "\"[CLIENT-VERSION]\"\n");
    fprintf(stderr, "\nExamples:\n");
    fprintf(stderr,
            "%s 123404@cucc 123456 eth0 \"CH\\x11V7.10-0213\" Oly5D62FaE94W7\n",
            argv[0]);
    fprintf(
        stderr,
...
void convert(char *dest, const char *src) {
  ...
  while (srcIndex < srcLength) {
    // 检查是否有十六进制转义序列
    if (src[srcIndex] == '\\' && srcIndex + 3 < srcLength &&
        src[srcIndex + 1] == 'x') {
      // 提取十六进制转义序列
      char hexSequence[3];
